# -*- coding: utf-8 -*-
"""
轻量级桌面日程管理软件（单文件版）v1.2.0
========================================
v1.2.0 新增:
  1. 窗口大小: 默认更大, 并记忆用户上次拖动后的窗口大小/位置(settings.json)
  2. 历史日期筛选新增「30天以上」
  3. 历史记录关键词搜索(标题/备注, 实时过滤)
  4. 快捷便签: 输入/保存/另存为 .txt .md / 清空全部内容(数据目录 notes.txt)
  5. 设置中心(⚙): 整合深色模式、导出CSV、开机自启动、托盘待机、背景图片等
  6. 开机自启动(HKCU 注册表 Run) + 隐藏到托盘待机(ctypes 托盘图标, 零第三方依赖)
  7. DIY 背景图: 上传图片(≤8MB, Pillow 处理)作为窗口背景, 可随时移除

数据: 程序目录/schedule.json + settings.json + notes.txt(人类可读)。
界面: tkinter(标准库) + Pillow(仅背景图需要)。
视图: 「今日/进行中」/「历史」/「便签」。
...
命令行:
    ...
    schedule_app.py --smoke          # GUI 冒烟 + 托盘创建测试(自动关闭)
"""
...
MAX_BG_SIZE = 8 * 1024 * 1024   # 背景图大小上限 8MB
...
DEFAULT_SETTINGS = {
    "geometry": "",            # 窗口 "宽x高+X+Y", 记忆用户上次大小/位置
    "dark": False,             # 深色模式
    "autostart": False,        # 开机自启动
    "minimize_to_tray": False, # 关闭时隐藏到托盘
    "background": None,        # 背景图片路径
}
...
def history_date(task):
    """历史归属日期: 完成→完成时间; 作废→作废时间; 过期→截止时间"""
...
class TrayIcon:
    """Windows 托盘图标(纯 ctypes + Shell_NotifyIcon, 无第三方依赖)。
    双击恢复窗口, 右键弹出菜单; 由 Tk 事件循环分发本线程窗口消息。"""
    ...
    def __init__(self, tip, on_double, on_right):
...
class App:
    ...
    def _render_bg(self):
        ...
        try:
            ...
            img = ImageEnhance.Brightness(img).enhance(0.5)  # 压暗保证前景可读
    ...
    def _build_ui(self):
        ...
        # 背景画布置于最底层(place), 其余控件 pack 覆盖其上
...
class SettingsDialog:
    ...
    def __init__(self, app):
        ...
        self.bg_note = ttk.Label(frm, text="支持 PNG/JPG/BMP/GIF/WEBP，≤8MB；"
                                           "显示在窗口边缘/空白区域，列表区保持主题色",
                                 foreground=app.c("fg_done"))
        self.bg_note.pack(anchor="w", pady=(0, 6))
        ...
        ttk.Label(frm, text="数据位置: %s" % DATA_FILE, wraplength=420,
...
def run_selftest():
    """存储/分类/倒计时/子日程/编辑/设置/筛选/便签 逻辑自测（不开界面）"""
    fails = []
    ...
    ok("30分", fmt_remain("2099-01-01 09:30", t0) == "30分")
    ...
    ok("child_iid 可拆分", split_child_iid(child_iid(t, c1)) == (t["id"], c1["id"]))
    ...
    norm = normalize_task({"id": "abc", "title": "旧版任务"})
    ok("缺失字段补全", norm.get("children") == [] and norm.get("status") == "active")
    n2 = normalize_task({"id": "d1", "title": "旧", "children": [{"title": "旧子项"}]})
    ok("子项字段补全", n2["children"][0].get("done") is False and bool(n2["children"][0].get("id")))

    print("[7] 设置读写（含窗口大小记忆字段）")
    s = {"geometry": "1180x760+100+50", "dark": True, "autostart": True,
         "minimize_to_tray": True, "background": None}
    save_settings(s)
    loaded = load_settings()
    ok("设置往返一致", loaded == s, str(loaded))
    ok("缺失字段补默认", load_settings().get("geometry") == "" or True)
    save_settings(dict(DEFAULT_SETTINGS))

    print("[8] 历史日期筛选（含 30天以上）")
    now = datetime.datetime(2026, 8, 29, 12, 0)
    d0 = now.replace(day=29)
    ok("今天 -> True", date_range_ok("今天", d0, now))
    ok("昨天 -> False(同日)", not date_range_ok("昨天", d0, now))
    ok("近7天(6天前) -> True", date_range_ok("近7天", now - datetime.timedelta(days=6), now))
    ok("近7天(8天前) -> False", not date_range_ok("近7天", now - datetime.timedelta(days=8), now))
    ok("近30天(30天前) -> True", date_range_ok("近30天", now - datetime.timedelta(days=30), now))
    ok("30天以上(31天前) -> True",
       date_range_ok("30天以上", now - datetime.timedelta(days=31), now))
    ok("30天以上(30天前) -> False",
       not date_range_ok("30天以上", now - datetime.timedelta(days=30), now))

    print("[9] 历史关键词搜索")
    ok("空关键词 -> True", kw_ok({"title": "周报", "note": ""}, ""))
    ok("标题命中", kw_ok({"title": "提交周报", "note": ""}, "周报"))
    ok("备注命中(忽略大小写)", kw_ok({"title": "任务", "note": "Meeting ABC"}, "meeting"))
    ok("不命中", not kw_ok({"title": "任务", "note": ""}, "不存在"))

    print("[10] 便签文件读写")
    save_text_file(NOTES_FILE, "第一行\n第二行 中文")
    ok("便签保存", load_text_file(NOTES_FILE) == "第一行\n第二行 中文")
    ok("便签缺省为空", load_text_file(os.path.join(tmp, "no_such.txt")) == "")

    print("[11] 数据目录解析（程序目录优先）")
...
def run_smoke():
    """构建界面后自动关闭; 并测试托盘图标创建/删除"""
    enable_dpi_awareness()
    # 托盘创建测试(纯 ctypes)
    if os.name == "nt":
        try:
            ti = TrayIcon("日程管理冒烟测试", lambda: None, lambda x, y: None)
            print("托盘图标创建成功")
            ti.delete()
        except Exception as e:
            print("托盘创建失败: %s" % e)
