# -*- coding: utf-8 -*-
"""
轻量级桌面日程管理软件（单文件版）v1.5.0
========================================
v1.5.0 修复:
  1. 半透明模式不再"携带"背景一起变淡: 背景图绘制在主窗口正后方的不透明背景窗口上,
     主窗口以半透明 alpha 覆盖其上 -> 背景图保持完整不透明度, 由半透明面板透出
  2. 开机自启动: 写入失败时以管理员权限(UAC)重试, 提权回调同时写 HKCU 与 HKLM,
     并回读校验提示结果
  3. 全局符号/句子插入修复: 点击后先把前台窗口还给唤起前的应用, 再发送输入,
     解决"召唤出来了但光标处无输出"的问题
v1.5.0 新增:
  4. 历史记录「复制为模板」: 以历史日程为模板复制一份, 修改日期后保存为新的常规日程
     (沿用标题/备注/周期/标记/置顶/子日程, 原历史记录保留)

v1.4.0(历史): 便签当前文件跟踪 / 符号·句子 DIY 与全局热键 / 事件 / 画板 /
半透明 / toast 提示 / 数据 data/ 子文件夹。
...
DEFAULT_SETTINGS = {
    ...
    "symbol_hotkey": "Ctrl+Shift+G",  # 符号小窗全局热键(Alt+Ctrl 常被输入法占用)
    "snippet_hotkey": "Ctrl+Shift+S", # 句子粘贴板全局热键
...
def make_template_from(task):
    """历史记录 → 新日程模板: 沿用标题/备注/周期/标记/置顶/子日程, 清空时间,
    生成全新 id 与 active 状态。"""
...
def set_autostart(enabled, hklm=False):
    """写/删开机自启动注册表项。hklm=True 时写 HKLM(所有用户, 需管理员)。
    普通用户写 HKCU 即可; 提权回调会同时写两者。"""
    if os.name != "nt":
        return
    import winreg
    if hklm:
...
def elevate_autostart(enabled):
    """以管理员身份(UAC)重新运行本程序完成自启动写入(HKCU + HKLM)"""
    if os.name != "nt":
        return
    try:
    ...
    try:
        ...
        class INPUT(ctypes.Structure):
            class _I(ctypes.Union):
                # union 需与最大成员 MOUSEINPUT(32 字节) 等宽, 否则 SendInput 报
                # ERROR_INVALID_PARAMETER(87), 字符不会真正输入
...
class App:
    ...
    # ---- 背景: 双窗口方案 ----
    # 背景图绘制在主窗口正后方的不透明背景窗口(bg_win)上, 主窗口以半透明
    # alpha 覆盖其上 -> 背景图保持完整不透明度, 半透明的面板将其透出。
    def _render_bg(self):
    ...
    def toggle_symbols(self):
        if getattr(self, "sym_win", None) is not None:
            try:
                ...
                self.sym_win = None
        tk, _, _, _, _ = need_tk()
        self._prev_fg = get_foreground_hwnd()          # 记录唤起前的活动窗口
        ...
        try:
            self.sym_win.attributes("-toolwindow", True)   # 尽量不抢占激活
        except Exception:
            pass
    ...
    def _insert_text(self, text):
        ...
        if w is not None:
            try:
                ...
                pass
        # 焦点在本程序之外(如其他软件): 隐藏小窗 -> 把前台还给原窗口 -> 输入 -> 重新显示
        ...
        if win is not None:
            try:
                ...
                pass
            set_foreground_hwnd(getattr(self, "_prev_fg", None))
            self.root.after(150, lambda: send_unicode_char(text))
    ...
    def on_history_copy_template(self):
        """历史记录 → 复制为新日程模板: 预填内容, 修改日期后保存为新的常规日程"""
        sel = self.tree_hist.selection()
        if not sel:
            self._info("请先在历史列表中选择一条记录。")
            return
        ...
        if task is None:
            return
        tmpl = make_template_from(task)
        AddDialog(self, task=tmpl, template=True)
...
class SettingsDialog:
    ...
    def _on_autostart(self):
        ...
        save_settings(self.app.settings)
        # 先尝试普通 HKCU 写入; 失败或未生效时以管理员身份重试(HKCU+HKLM)
        try:
            set_autostart(enabled)
            if get_autostart() == enabled:
                self.app._toast("开机自启动已%s" % ("开启" if enabled else "关闭"))
                return
        except Exception:
            pass
        elevate_autostart(enabled)
        self.app._info("已请求管理员权限完成开机自启动设置（请在弹出的 UAC 窗口中确认）。\n"
...
class AddDialog:
    ...
    template=True 时以 task 为模板预填, 保存后作为全新常规日程加入。"""
    ...
    def _save(self):
        ...
        if self.template:
            # 以历史为模板: 沿用内容/周期/标记/子日程, 新 id, 填入新日期, 作为新日程加入
...
def run_selftest():
    ...
    print("[11] 历史复制为模板")
    src = new_task("历史任务", note="备注A", deadline="2099-01-01 09:00",
                   repeat="weekly", start="2099-01-01 08:00")
    src["flag"] = "red"
    src["pinned"] = True
    src["children"] = [new_child("子1"), new_child("子2")]
    src["status"] = "done"
    src["completed_at"] = now_iso()
    tmpl = make_template_from(src)
    ok("模板为新任务(active/新id)", tmpl["status"] == "active"
       and tmpl["id"] != src["id"])
    ok("沿用标题/备注/周期", tmpl["title"] == "历史任务"
       and tmpl["note"] == "备注A" and tmpl["repeat"] == "weekly")
    ok("沿用标记/置顶/子日程", tmpl["flag"] == "red" and tmpl["pinned"] is True
       and len(tmpl["children"]) == 2)
    ok("时间清空(供重新填写)", tmpl["start"] is None and tmpl["deadline"] is None)
...
def main():
    ...
    if "--smoke" in sys.argv:
        sys.exit(run_smoke())
    if "--autostart-set" in sys.argv:      # 提权回调: 写 HKCU + HKLM(需管理员) 后退出
        i = sys.argv.index("--autostart-set")
        if i + 1 < len(sys.argv):
            val = sys.argv[i + 1] == "1"
            try:
                set_autostart(val)            # HKCU
            except Exception:
                pass
            try:
                set_autostart(val, hklm=True)  # HKLM(所有用户, 提权后可写)
            except Exception:
                pass
        sys.exit(0)
