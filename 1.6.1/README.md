...
- **工具功能**：`tools/` 文件夹放外部 exe / **.lnk 快捷方式 / .url 网页链接**，「工具」页一键启动（.url 用默认浏览器打开），可选**以管理员身份启动**，支持**关键词搜索过滤**
...
- 事件截止偏移分钟（起始 12:00 + 截止 150 → 14:30）；日程模板列表（DIY）；快捷时间可 DIY
  且作用于光标所在字段；添加/子日程对话框固定居中、底部确认键始终可见
- **周期日程完成推进**：标记完成即自动推进到下一周期，并记录**已完期数**与**上次完成时间**
  （主界面与详情中直接显示，如「已完3期 · 上期完成 …」）
...
### 工具
`<程序目录>\tools\` 下的 exe / .lnk 快捷方式 / .url 网页链接会出现在「工具」页，
选中后「▶ 启动」（.url 用默认浏览器打开），支持**关键词搜索**过滤列表、
「以管理员身份启动」、重新扫描 / 打开文件夹。

### 周期日程完成
周期日程点「完成」后不会消失，而是**自动推进到下一周期**，并记录**已完期数**
与**上次完成时间**；主界面时间列与详情对话框直接显示（如「已完3期 · 上期完成 2026-08-30 09:00」）。
...
| `<程序目录>\tools\` | 外部工具 exe / .lnk 快捷方式 / .url 网页链接（工具页调用） |
...
|---|---|
| 模块化拆分（schedapp 包） | ✅ |
| 工具页(.exe/.lnk/.url+搜索+管理员) / 便签 IDE(分类+行标+帮助) / 日历 / MD 渲染 / LaTeX 悬停 / 偏移分钟 / 模板 / 快捷时间DIY | ✅ v1.6.0 |
| 周期完成推进+期数记录 / 添加对话框按钮修复 | ✅ v1.6.0 |
