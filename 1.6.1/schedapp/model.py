...
def normalize_task(t):
    for k, v in {"id": uuid.uuid4().hex, "title": "", "note": "", "start": None,
                 ...
                 "repeat": None, "pinned": False, "flag": None,
                 "repeat_count": 0, "last_completed_at": None}.items():
        t.setdefault(k, v)
...
def complete_recurring(task, now=None):
    """完成周期日程: 已完期数 +1, 记录本次完成时间, 并把截止推进到下一周期。
    返回推进后的 deadline 文本。"""
    now = now or datetime.datetime.now()
    task["repeat_count"] = (task.get("repeat_count") or 0) + 1
    task["last_completed_at"] = now_iso()
    dt = parse_dt(task.get("deadline"))
    if dt is not None:
        nd = next_occurrence(dt, task["repeat"], now)
        task["deadline"] = nd.strftime("%Y-%m-%d %H:%M")
        return task["deadline"]
    return None


def repeat_summary(task):
    """周期日程状态摘要: 已完 N 期 / 上次完成时间"""
    n = task.get("repeat_count") or 0
    last = task.get("last_completed_at")
    parts = []
    if n:
        parts.append("已完%d期" % n)
    if last:
        parts.append("上期完成 %s" % fmt_dt(last))
    return " · ".join(parts)
