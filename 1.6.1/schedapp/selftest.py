...
def run_selftest():
    ...
    rep = model.new_task("周期任务", deadline="2026-08-30 18:00", repeat="weekly")
    nd = model.complete_recurring(rep, datetime.datetime(2026, 9, 2, 10, 0))
    ok("完成后截止推进到下一周", nd == "2026-09-06 18:00", str(nd))
    ok("记录已完期数与完成时间",
       rep["repeat_count"] == 1 and bool(rep["last_completed_at"]))
    ok("摘要: 已完1期", "已完1期" in model.repeat_summary(rep),
       model.repeat_summary(rep))
